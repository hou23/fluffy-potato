...
/**
 * Created by marshall.houz on 2018/5/2.
 * <p>
 * 在分布式系统中生成全局唯一的ID:
 *
 * 1. UUID, 占用32位太长, 且无序. 入库时性能比较差(见B+树索引的分裂)
 * 2. 数据库自增主键, 设置每个分库的初始值和步长. ID生成对数据库严重依赖,
 * 不但影响性能, 而且数据库挂了服务就不可用
 *
 * 3. snowflake, 生成ID时不依赖DB, 完全在内存生成, 高性能高可用
 * ID呈趋势递增, 后续插入索引树的时候性能较好
 * 但依赖于系统时钟的一致性. 如果某台机器的系统时钟回拨, 有可能造成ID冲突, 乱序
 */
